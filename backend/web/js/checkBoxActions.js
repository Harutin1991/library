    ...
    $('.action-block>input').click(
        function () {
            ...
            if (checked[0].length > 0) {
                var ids = checked[0];
                var index = checked[1];
                if(action == 'is_annuled' || action == 'accepted'){
                    lastUri = "check-box";
                    uri += lastUri;
                    if(action == 'is_annuled'){
                        status = 1;
                    }if(action == 'accepted'){
                        status = 0;
                    }

                    var data = {data: {id: ids, is_annuled: status}};
                    if(ajaxQuery(uri, data) === true){
                        ...
                        $('#admin-alerts').children('div').text('successfully '+ action);
                        ...
                        $('#admin-alerts').children('div').text('error is not '+action);
                    }
...
var kit = $( "#product-category_id option:selected" ).text();
    if(kit === "Kits"){
        $(".field-product-kits_id").css({"display":"block"});
    }else{
        $(".field-product-kits_id").css({"display":"none"});
    }
});
kit = $( "#product-category_id option:selected" ).text();
if(kit === "Kits"){
    $(".field-product-kits_id").css({"display":"block"});
}else{
    $(".field-product-kits_id").css({"display":"none"});
...
//    return "<img class='flag' src='uploads/images/29.png'/>" + state.text;
//}
//$("#product-kits_id").select2({
//    formatResult: format,
